...
class RecommendationEngine:
    ...
    def _score_team_needs(
        ...
        Checks roster against required slots:
          - 11 hitter slots: C, 1B, 2B, 3B, SS, MI, CI, 4×OF, U
        ...
        TOTAL_HITTER_SLOTS = 11  # sum of above
        ...
        if is_pitcher:
            ...
            # undervalued.  Pitchers are 9/20 of the roster — use similar per-slot
        ...
        # --- Bench-only penalty: don't fill bench until the last round ---
        ...
        final_round = draft_state.roster_size

        if is_hitter and hitter_count >= TOTAL_HITTER_SLOTS:
            if current_round < final_round:
                need_score -= 300
                reasoning_parts.append("BENCH ONLY — save for last round")
            else:
                reasoning_parts.append("Bench pick (final round)")
