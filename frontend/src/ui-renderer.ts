...
export class UIRenderer {
    ...
    renderPlayerModal(player: Player, onDraft: (player: Player) => void, draftComplete: boolean): void {
        ...
        let statsHtml = '';
        const fmt = (v: number | undefined | null, decimals = 0): string => {
            if (v == null) return '—';
    ...
    renderDraftBoard(data: {
        board: Array<{ pick_number: number; round: number; team_name: string; player_name: string; position: string }>;
        total_teams: number;
        roster_size: number;
    }, teams: string[], myTeam: string): void {
        ...
        const picksByRound: { [round: number]: { [team: string]: { player_name: string; position: string } } } = {};
    ...
    renderCategoryRankings(data: {
        category_totals: { [team: string]: { [cat: string]: number } };
        category_points: { [cat: string]: { [team: string]: number } };
        category_rankings: { [cat: string]: string[] };
        final_rankings: string[];
    }, myTeam: string): void {
        const select = document.getElementById('category-rank-select') as HTMLSelectElement | null;
